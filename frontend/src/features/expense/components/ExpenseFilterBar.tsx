...
import type { ExpenseFilterParams, Expense } from '../types/expense.type.ts';
...
interface ExpenseFilterBarProps {
  onFilterChange: (filters: ExpenseFilterParams) => void;
  expenses: Expense[];
  isLoading?: boolean;
}
...
export const ExpenseFilterBar: React.FC<ExpenseFilterBarProps> = ({
  ...
  useEffect(() => {
    ...
    if (isAdmin) {
      fetchAdminUsersApi({ page_size: 100 })
        ...
        .catch((err) => console.error('Failed to fetch admin users for expense filter:', err));
    }
  }, [fetchBankAccounts, isAdmin]);

  // Aggregate dynamic categories from expenses
  ...
  const applyFilters = (
    ...
    const filterPayload: ExpenseFilterParams = {
    ...
    <div className="bg-[#1e252e] border border-white/5 rounded-2xl p-4 sm:p-5 mb-6 shadow-xl space-y-4">
      ...
      <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3">
        ...
        <div className="relative flex-1">
          ...
          <input
            type="text"
            placeholder="Search expenses by category, description, or account..."
            value={search}
            onChange={(e) => {
              setSearch(e.target.value);
              applyFilters({ search: e.target.value });
            }}
            className="w-full bg-[#161c24] border border-white/10 rounded-xl pl-9 pr-9 py-2 text-xs sm:text-sm text-white placeholder-gray-500 outline-none focus:border-rose-500 transition-all"
          />
          {search && (
            <button
              onClick={() => {
        ...
        <div className="flex items-center gap-1.5 overflow-x-auto pb-1 scrollbar-none shrink-0">
          <span className="text-[11px] font-semibold text-gray-400 uppercase tracking-wider flex items-center gap-1 shrink-0 mr-1">
            <RiCalendarEventLine className="text-rose-400" />
          </span>
          {[
            ...
            <button
              ...
              className={`text-xs px-2.5 py-1.5 rounded-lg font-medium transition-all duration-200 shrink-0 cursor-pointer ${
                preset === p.id
                  ? 'bg-rose-500/20 text-rose-300 border border-rose-500/40 shadow-[0_0_10px_rgba(244,63,94,0.15)]'
        ...
        >
          <RiFilter3Line className={isLoading ? 'animate-spin text-rose-400' : ''} />
          <span>Filters</span>
          {activeCount > 0 && (
      ...
      {/* ─── ROW 2: Primary Dropdowns (Category, Account, Sort, Admin Scope) ─── */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 pt-2 border-t border-white/5">
        ...
        <div>
          <label className="block text-[11px] font-medium text-gray-400 mb-1 flex items-center gap-1">
            <RiPriceTag3Line className="text-rose-400" /> Expense Category
          </label>
          <select
            ...
            className="w-full bg-[#161c24] border border-white/10 rounded-xl px-3 py-2 text-xs text-gray-200 outline-none focus:border-rose-500 transition-colors cursor-pointer"
        ...
        <div>
          <label className="block text-[11px] font-medium text-gray-400 mb-1 flex items-center gap-1">
            <RiBankCardLine className="text-cyan-400" /> Paid From Account
          </label>
          <select
        ...
        {isAdmin ? (
          <div>
            ...
            <select
              ...
              style={{ colorScheme: 'dark' }}
            >
              <option value="me">👤 My Expenses Only</option>
          ...
          <div>
            <label className="block text-[11px] font-medium text-gray-400 mb-1 flex items-center gap-1">
              <RiMoneyDollarCircleLine className="text-rose-400" /> Min - Max Amount
            </label>
            <div className="flex items-center gap-2">
              <input
                ...
                className="w-1/2 bg-[#161c24] border border-white/10 rounded-xl px-2.5 py-2 text-xs text-white placeholder-gray-500 outline-none focus:border-rose-500"
              />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 pt-3 border-t border-white/5 animate-fadeIn">
          ...
          <div>
            ...
            <input
              ...
              className="w-full bg-[#161c24] border border-white/10 rounded-xl px-3 py-1.5 text-xs text-white outline-none focus:border-rose-500"
              style={{ colorScheme: 'dark' }}
            />
          </div>
          ...
          <div className="flex items-end">
            <button
              ...
              className="w-full py-2 bg-rose-500/20 hover:bg-rose-500/30 text-rose-300 border border-rose-500/30 rounded-xl text-xs font-bold transition-all cursor-pointer"
            >
              Apply Date Range
            </button>
          </div>
          ...
          {isAdmin && (
            <div>
              <label className="block text-[11px] font-medium text-gray-400 mb-1 flex items-center gap-1">
                <RiMoneyDollarCircleLine className="text-rose-400" /> Amount Range (₹)
              </label>
              <div className="flex items-center gap-2">
                <input
                  type="number"
                  ...
                  className="w-1/2 bg-[#161c24] border border-white/10 rounded-xl px-2.5 py-1.5 text-xs text-white placeholder-gray-500 outline-none focus:border-rose-500"
                />
                <span className="text-gray-500 text-xs">-</span>
                <input
                  type="number"
      ...
      {isFilterActive && (
        <div className="flex items-center justify-between gap-2 pt-2 border-t border-white/5 text-xs">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {preset !== 'all_time' && (
              <span className="bg-rose-500/10 border border-rose-500/30 text-rose-400 px-2 py-0.5 rounded-full text-[11px]">
